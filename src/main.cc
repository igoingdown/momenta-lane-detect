...
//     cvCvtColor(raw_mat, imDisplay, CV_RGB2BGR);
    ...
    if (lanesConf.ransacSpline)
    {
      ...
      string write_image_dir = "../caltech-lanes/image_with_lanes/";
      string image_name(filename + 24);
      cv::Mat dst;
      dst = cv::Mat(imDisplay);
      cv::imwrite(write_image_dir + image_name, dst);
      // cout << write_image_dir + image_name << endl;
    }
    // show?
